...
function update() {
	...
	*/
	result.forEach(function(center) {
		...
		*/
		for (var i = 0, n=0; n<N; i += (2*Math.PI / N) + (rand() - 0.5) * 0.4,n++) {
			/*
			...
			*/
			var radius = rand() * delta + 1.2*Math.sqrt(2);
...
window.onload = function() {
	$("body").append('<ul>\
			<li>Вариант: <input class="form-control" type="number" id="seed" min="1" max="50" value="1" onchange="$(' + "'#line'" + ').val(String()); update()" /></li>\
			<li>Ширина (30-1000): <input class="form-control" type="number" id="W" min="30" max="1000"  value="50" onchange="$(' + "'#line'" + ').val(String()); window.onresize()" /></li>\
			<li>Высота (30-1000): <input class="form-control" type="number" id="H" min="30" max="1000"  value="50" onchange="$(' + "'#line'" + ').val(String()); window.onresize()" /></li>\
			<li>Плотность препятствий:<br/>\
				<select class="form-control" id="density" onchange="$(' + "'#line'" + ').val(String()); update()">\
					<option value="1">Разреженно</option>\
					<option value="2">Нормально</option>\
					<option value="3">Густо</option>\
				</select>\
			</li>\
			<li>Размерность препятстий (3-8): <input class="form-control" type="number" id="N" min="3" max="8"  value="5" onchange="$(' + "'#line'" + ').val(String()); window.onresize()" /></li>\
			<li>Исходные данные:<br /><textarea class="form-control" id="result" /></li>\
			<li>Решение:<br /><textarea class="form-control" id="line" onchange="upshow()" /></li>\
			<li><button type="button" style="margin-top:10px;" class="btn btn-info" data-toggle="modal" data-target="#explanation">Пояснения</button></li>\
		</ul>');
 ...
 },
 {
  ...
  "y2": 20
	...
	$("body").append("\
<div id=\"explanation\" class=\"modal fade\" role=\"dialog\">\
  <div class=\"modal-dialog\"></div></div>");
	$(".modal-dialog").load("explanation.htm");
}